...
def restart_from_checkpoint_domain(ckp_paths, run_variables=None, **kwargs):
    """
    Re-start from checkpoint
    """
    # look for a checkpoint in exp repository
    ...
    mode = kwargs["mode"]
    if mode == "train":
        for key,_ in checkpoint["dis_state_dict"].items():
            if not "prototypes" in key  and not "projection" in key:# and not "projection_head" in key: #(******)
                model.state_dict()[key].copy_(checkpoint["dis_state_dict"][key])
    elif mode == "test":
        for key,_ in checkpoint["dis_state_dict"].items():
            model.state_dict()[key].copy_(checkpoint["dis_state_dict"][key])
...
def restart_from_checkpoint_alex(ckp_paths, run_variables=None, **kwargs):
    """
    Re-start from checkpoint
    """
    # look for a checkpoint in exp repository
    ...
    model = kwargs["state_dict"]
    for key,_ in checkpoint.items():
       if 'top_layer' not in key and 'sobel.0' not in key:# and not "projection_head" in key: #(******)
           model.state_dict()['module.'+key.replace('module.', '')].copy_(checkpoint[key])
    logger.info("=> loaded {} from checkpoint '{}'".format(key, ckp_path))
